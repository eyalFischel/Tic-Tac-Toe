...
class GameState(BaseModel):
    ...
    def add_player(self, player: Player):
        ...
        if player.nickname not in self.players and len(self.players) < 2:
            ...
            raise ValueError(
                "Player already exists in the game or the room is already full."
            )
    ...
    def play(self, play: Play):
        ...
        if self.board[play.position] != " ":
            ...
            raise ValueError(
                f"Position already taken. Choose another position: {free_positions}"
            )
    ...
    def _check_for_win(self, nickname: str):
        ...
        # Columns
        for pos in range(3):
            if [self.board[pos], self.board[pos + 3], self.board[pos + 6]] == [
                player_symbol
            ] * 3:
                return True
